let jsonHeroes = `[
    {
      ...
      "longtext": "по популярности человек-летучая мышь может сравниться только с Суперменом. Его образ кажется очень мистическим и мрачным: черный костюм с развевающимся плащом, устрашающий Бэтмобиль, штаб-квартира в сырой пещере. Его биография настолько же темна, как и образ. В детстве у него на глазах убили родителей, и юный Брюс поклялся всеми силами защищать родной Готэм от преступности.     Брюс знаменит, несмотря на пожилой возраст: первый выпуск с его участием вышел аж в 1939 году. С каждым годом популярность только крепнет. Большую роль в этом сыграли экранизации: «Темный рыцарь», сериал «Готэм» и несколько одноименных картин. Главная причина такой любви среди фанатов — мысль, что каждый может стать Бэтменом.    Если подумать, это один из немногих героев, кто добился своего положения за счет ума и упорства, а не за счет мутаций.",
...
function saveVotes(votes = {}) {
    ...
    window.localStorage.setItem(VOTES_STORAGE_KEY, votesString)
}

function saveHeroVote(id, vote) {
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    function generateVoteSelect(id, vote = 0) {
        let result = `<select onChange="saveHeroVote('${id}', this.value)">
                        <option value="0"></option>`
        for (let i = 1; i <= 10; i++) {
            result += `<option value="${i}" ${i === vote ? 'selected' : ''}>${i}</option>`
        }
        result += `</select>`
        return result
    }
    ...
    for (let hero of heroes) {
        ...
        const heroVote = votes[hero.id] || 0

        heroesContent += `<div class='card'>
              <h2>${hero.name}</h2>` +
            `<div>Вселенная: ${hero.universe}</div> 
                   <div>Альтер эго: ${hero.altername} </div> 
              <div>Род деятельности: ${hero.occupation}</div> 
                   <div>Друзья: ${hero.friends} </div> 
              <div>Суперспособности: ${hero.superpowers}</div>
              <div>Подробнее: ${hero.longtext}</div>  
              <img src='${hero.picture}'> 
                   <h3>Поставьте оценку в вашем рейтинге</h3>
              ${generateVoteSelect(hero.id, heroVote)} </div>`
    };
    document.getElementById("heroContainer").innerHTML = heroesContent
